...
- **视觉效果**: DWM API (毛玻璃效果)
- **定时器**: 多媒体定时器 (timeSetEvent) 替代 WM_TIMER
...
### 编译注意事项
由于 CMake 4.2.1 的 cmake_cl_compile_depends 在处理中文 MSVC 输出时出现乱码导致临时文件错误，需要使用 compile_gui.bat 直接调用 cl.exe 和 link.exe 进行编译，绕过 CMake 的依赖检查。编译脚本设置 VSLANG=1033 强制英文输出。
...
| `g_oscPauseExpandH` | OSC暂停提示水平展开 |
| `g_oscPauseExpandV` | OSC暂停提示垂直展开 |
| `g_oscPauseCloseAnim` | OSC暂停提示关闭动画 |

---

## 极简模式

极简模式下所有 OSC 消息简化为最短形式：

| 场景 | 正常模式 | 极简模式 |
|------|----------|----------|
| 启动 | `VRChatlyricsdisplay
这是一条测试消息哦~
用于确保OSC消息可以正常使用` | `已启动` |
| 暂停 | `正在暂停 OSC 发送... 请等待 30 秒` | `已暂停` |
| 退出 | `VRCLyricsDisplay
欢迎下次使用哦~` | `·` |
| 性能监控 | 多行详细性能信息 | `C X% R X% G X%` (单行) |

性能监控支持通过 `display_modules` 调整显示顺序。
...
*Generated by iFlow CLI on 2026-03-14*